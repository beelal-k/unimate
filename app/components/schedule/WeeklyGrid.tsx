...
function ClassBlock({
  ...
  // Figure out if this is a shared class to display avatars
  const isShared = classItem.sharedWith && classItem.sharedWith.length > 0;
  // Show avatar if ownerInitials is set (always in shared schedule mode)
  const showAvatar = !!classItem.ownerInitials;
  // All participants = owner + sharedWith
  const participants = isShared ? [
    { initials: classItem.ownerInitials || 'Me', id: classItem.ownerUserId || 'me' },
    ...classItem.sharedWith!.map(s => ({ initials: s.initials, id: s.userId }))
  ] : showAvatar ? [
    { initials: classItem.ownerInitials || 'Me', id: classItem.ownerUserId || 'me' }
  ] : [];
    ...
    >
      ...
      <View style={{ flex: 1, justifyContent: 'flex-start', paddingTop: 2 }}>
        {(!!classItem.code || !!classItem.section) && (
          ...
          >
            {[classItem.code, classItem.section].filter(Boolean).join(' · ')}
          </Text>
        )}
        <Text
      ...
      {/* Avatars row */}
      {showAvatar && participants.length > 0 && (
        <View style={{ position: 'absolute', bottom: 4, right: 4, flexDirection: 'row' }}>
          {participants.slice(0, 3).map((p, i) => (
            <View key={p.id} style={{ marginLeft: i > 0 ? -6 : 0 }}>
              <Avatar 
                initials={p.initials} 
                size={20} 
                color={p.id === 'me' ? '#0A0A0A' : undefined}
              />
            </View>
          ))}
          {participants.length > 3 && (
            <View style={{ marginLeft: -6, width: 16, height: 16, borderRadius: 8, backgroundColor: '#E0E0E0', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: '#FFF' }}>
              <Text style={{ fontSize: 7, fontFamily: 'Inter_700Bold', color: '#6E6E6E' }}>+{participants.length - 3}</Text>
            </View>
          )}
        </View>
      )}
