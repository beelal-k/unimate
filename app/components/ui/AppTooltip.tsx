...
export function AppTooltip() {
  ...
  // Only show when inside the tabs group — never on auth or index screens.
  useEffect(() => {
    if (!isInTabs) {
      setIsVisible(false);
      return;
    }
    SecureStore.getItemAsync('has_seen_tooltips').then((hasSeen) => {
  ...
  const handleNext = async () => {
    if (currentStep < TOOLTIP_STEPS.length - 1) {
      ...
      const nextRoute = `/${TOOLTIP_STEPS[nextIndex].path}`;
